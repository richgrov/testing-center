...
import { Availability, OwnedScheduling, Scheduling, TimeWindow } from "@/components/scheduling/types";
...
export default function EditTestSlotPage() {
  ...
  const [enrollment, setEnrollment] = useState<TestEnrollment | null | undefined>(null);
  ...
  useEffect(() => {
    ...
    setEnrollment(null);
    const subscription = TestEnrollments.subscribe(params.enrollmentId!!, event => setEnrollment(event.record as TestEnrollment ?? false), { expand: "test" });
    subscription
      .then(() => TestEnrollments.getOne(params.enrollmentId!!, { expand: "test" })).then(record => setEnrollment(record as TestEnrollment ?? false))
      .catch(e => { console.error(e); setEnrollment(null); });
  ...
  const [desiredScheduling, setDesiredScheduling] = useState<Desired | null>(null);
  ...
  useEffect(() => {
    ...
    TestingCenterHours.getList(1, 1000, {
      filter: pocketBase.filter("(opens <= {:myOpenValue} && closes >= {:myOpenValue}) || (opens <= {:myCloseValue} && closes >= {:myCloseValue}) || ({:myOpenValue} <= opens && {:myCloseValue} >= opens) || ({:myOpenValue} <= closes && {:myCloseValue} >= closes)", { myOpenValue, myCloseValue }),
    }).then(result => {
      for (const item of result.items) {
        selectedHours.set(item.id, item as TestingCenterHours);
      }
      if (result.items.length > 0) setAvailability(Array.from(selectedHours.values()));
    });
    const subscription = TestingCenterHours.subscribe<TestingCenterHours>("*", event => {
      if (event.action === "delete" && selectedHours.delete(event.record.id)) setAvailability(Array.from(selectedHours.values()));
      else {
        const opens = parsePocketbaseDate(event.record.opens);
        const closes = parsePocketbaseDate(event.record.closes);
        const relevant = (opens <= myOpenValue && closes >= myOpenValue) || (opens <= myCloseValue && closes >= myCloseValue) || (myOpenValue <= opens && myCloseValue >= opens) || (myOpenValue <= closes && myCloseValue >= closes);
        var updated = false;
        if (relevant) {
  ...
  interface MyScheduling extends Scheduling { day: number; }
  ...
  useEffect(() => {
    ...
    function entryForEnrollment(enrollment: TestEnrollment): MyScheduling | null {
      if (enrollment.start_test_at == null || enrollment.start_test_at === "") return null;
      const opens = parsePocketbaseDate(enrollment.start_test_at);
      const closes = new Date(opens.valueOf() + (enrollment.duration_mins * millisecondsInMinute));
      const relevant = (opens <= myOpenValue && closes >= myOpenValue) || (opens <= myCloseValue && closes >= myCloseValue) || (myOpenValue <= opens && myCloseValue >= opens) || (myOpenValue <= closes && myCloseValue >= closes);
      ...
      const startMins = (opens.valueOf() - day.valueOf()) / millisecondsInMinute;
      return {
        day: day.valueOf(),
        start: startMins,
    ...
    const filterCloseValue = new Date(myCloseValue.valueOf() + millisecondsInDay)
    TestEnrollments.getList<TestEnrollment>(1, 100000, {
      filter: pocketBase.filter("{:myOpenValue} <= start_test_at && {:myCloseValue} >= start_test_at", { myOpenValue: filterOpenValue, myCloseValue: filterCloseValue }),
    }).then(result => {
    ...
    const subscription = TestEnrollments.subscribe<TestEnrollment>("*", event => {
      if (event.action === "delete" && tracked.delete(event.record.id)) setExisting(Array.from(tracked.values()));
      else {
        var updated = false;
  ...
  function mintSchedulings<T>(startDate: Date, endDate: Date, map: Map<number, T[]>, transform: (window: TimeWindow) => T) {
  ...
  const timeDetails = useMemo(() => {
    ...
    for (const hours of availability) {
      mintSchedulings(parsePocketbaseDate(hours.opens), parsePocketbaseDate(hours.closes), days, (window) => ({ ...window, seats: hours.seats }));
    }
  ...
  const ownedSchedulings = useMemo(() => {
    ...
    if (enrollment != null && enrollment.start_test_at != null && enrollment.start_test_at !== "") {
      const start = parsePocketbaseDate(enrollment.start_test_at);
      const end = new Date(start.valueOf() + (enrollment.duration_mins * millisecondsInMinute));
      mintSchedulings(start, end, mySchedulings, (window) => ({ ...window, confirmed: true }));
    }
    if (desiredScheduling != null) {
      const start = new Date(desiredScheduling.startAtDate);
      const end = new Date(desiredScheduling.startAtDate + (desiredScheduling.minutes * millisecondsInMinute));
      mintSchedulings(start, end, mySchedulings, (window) => ({ ...window, confirmed: false }));
    }
    return mySchedulings;
  }, [enrollment?.start_test_at, enrollment?.duration_mins, desiredScheduling?.startAtDate, desiredScheduling?.minutes]);
  ...
  function submitDesiredInput() {
    ...
    console.log(pocketBase.filter("{:date}", { date: new Date(desiredScheduling.startAtDate) }));
  ...
  else if (enrollment === undefined) return <main>Test not found. Contact Christian Grey.</main>;
  else {
    return <main className="flex flex-col gap-4">
      <p className="text-center">
        Hello {enrollment.canvas_student_name}!<br/>
        <span className="text-3xl">Lets get you signed up for your {enrollment.expand.test.name}.</span>
      </p>
      <div className="flex flex-row wrap justify-center gap-4">
        <div className="flex-grow" style={{ border: "4px solid orange" }}>
          <h1 className="text-2xl"><u>Current Slot</u></h1>
          <p>
            {
              enrollment.start_test_at != null && enrollment.start_test_at != "" ? <>
                {formatDate(parsePocketbaseDate(enrollment.start_test_at))}<br />
        ...
        {desiredScheduling != null ? (
          <div className="flex-grow" style={{ border: "3px dashed orange" }}>
            <h1 className="text-2xl"><u>New Slot</u></h1>
            <p>
                {formatDate(new Date(desiredScheduling.startAtDate))}<br />
                Duration: <input value={desiredScheduling.minutes} onChange={e => {
                  const parsed = parseInt(e.target.value);
                  if (isNaN(parsed)) return;
                  setDesiredScheduling({ ...desiredScheduling, minutes: Math.min(parsed, desiredScheduling.maxMinutes) })
                }} className="w-8 border border-black" /> Minutes<br />
      ...
      <div className="flex flex-row nowrap w-full overflow-x-auto">
        {timeDetails.map(day => {
          ...
          return <VerticalSchedulingTimeline
            key={day[0]}
            ...
            update={bits => setDesiredScheduling({
...
function formatDate(date: Date) {
  ...
  if (hours === 0) hours = 12;
  return `${DAYS[date.getDay()]}, ${MONTHS[date.getMonth()]} ${date.getDate()} at ${String(hours).padStart(2, "0")}:${String(date.getMinutes()).padStart(2, "0")} ${date.getHours() >= 12 ? "PM": "AM"}`;
}
